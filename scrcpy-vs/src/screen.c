...
int video_arrive(byte* image_buff, int buff_size);
bool fill_jpeg_buff(AVFrame *frame) {
  if (window_need_show()) {
    return false;
  }
  ...
  video_arrive(packet.data, packet.size);
#if 0
  FILE *JPEGFile;
  char JPEGFName[256];
  static int i = 0;
